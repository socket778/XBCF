                    ...
                    bool b_scaling)
{
  ...
  for (size_t sweeps = 0; sweeps < state->num_sweeps; sweeps++)
  {
    ...
    for (size_t tree_ind = 0; tree_ind < state->num_trees_vec[0]; tree_ind++)
    {
      ...
      // if (state->z[0] == 1){
      //   cout << "sweeps " << sweeps << " tree " << tree_ind << " resid = " << (*state->y_std)[0] - state->a * state->mu_fit[0] - state->b_vec[1] * state->tau_fit[0] << " theta = " << (*(x_struct_ps->data_pointers[tree_ind][0]))[0] << endl;
      // }
      // else{
      //   cout << "sweeps " << sweeps << " tree " << tree_ind << " resid = " << (*state->y_std)[0] - state->a * state->mu_fit[0] - state->b_vec[0] * state->tau_fit[0] << " theta = " << (*(x_struct_ps->data_pointers[tree_ind][0]))[0] << endl;
      // }
